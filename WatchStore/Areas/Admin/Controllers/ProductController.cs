    ...
    public class ProductController : BaseController
    {
        ...
        public ActionResult Index(string searchString)
        {
            ...
            if (!String.IsNullOrEmpty(searchString))
            {
                ...
                query = query.Where(m =>
                    m.ProductName.ToLower().Contains(searchString) ||
                    m.CategoryName.ToLower().Contains(searchString) ||
                    m.ProductPrice.ToString().Contains(searchString) ||
                    m.ProductPriceSale.ToString().Contains(searchString)
                );
            }

            // Sắp xếp theo ngày tạo mới nhất
            var list = query.OrderByDescending(m => m.ProductCreated_At);
        ...
        {
            // Tạo query cơ bản cho các sản phẩm đã xóa
            var query = from p in db.Products
                        join c in db.Categorys
                        on p.CateID equals c.Id
                        where p.Status == 0
            ...
            if (!String.IsNullOrEmpty(searchString))
            {
                ...
                query = query.Where(m =>
                    ...
                    m.ProductPrice.ToString().Contains(searchString) ||
                    m.ProductPriceSale.ToString().Contains(searchString) ||
                    m.ProductDiscount.ToString().Contains(searchString)
                );
            }
