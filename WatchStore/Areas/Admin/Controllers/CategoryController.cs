...
namespace WatchStore.Areas.Admin.Controllers
{
    public class CategoryController : BaseController
    {
        ...
        public ActionResult Index(string searchString)
        {
            ...
            // Khởi tạo query
            var query = db.Categorys.Where(m => m.Status != 0);
            ...
            {   
                ...
                query = query.Where(m => m.Name.ToLower().Contains(searchString) ||
                                        m.Slug.ToLower().Contains(searchString));
            }
            ...
            // Lưu lại searchString để hiển thị trong view
        ...
        {
            ViewBag.Title = "Danh sách các loại sản phẩm đã xóa";

            // Khởi tạo query cơ bản cho các category đã xóa
            var query = db.Categorys.Where(m => m.Status == 0);
            ...
            var model = query.OrderByDescending(m => m.Created_at).ToList();

            // Populate ViewBag.GetAllCategory với tất cả categories
            ViewBag.GetAllCategory = db.Categorys.ToList();

            // Lưu searchString vào ViewBag để hiển thị trong view
            ViewBag.SearchString = searchString;
